...
def main(data_root, pred_path, vis_path):
    ...
    print(bboxes.shape, scores.shape, cls_arr.shape)

    for i in range(bboxes.shape[0]):
        if i < 5:
            print(bboxes[i], scores[i], cls_arr[i])
    ...
    # 构建BEV视图的画布 1000 * 1000
    canvas = np.zeros((int(canva_size), int(canva_size), 3), dtype=np.uint8)

    ## 绘制中心点和距离
    center_ego = (0, 0)
    # (0点x坐标 + 50范围) / 25 (显示图像的一般范围就是25) * 1000(放大到像素)
    center_canvas = int((center_ego[0] + show_range) / show_range / 2.0 * canva_size)
    
    cv2.circle(canvas, center=(center_canvas, center_canvas), radius=5, color=(255, 255, 255), thickness=0)
    ...
    for rid in sort_ids:
        ...
        # draw_boxes_indexes_bev索引 [(0, 1), (1, 2), (2, 3), (3, 0)]
...
if __name__ == '__main__':
    ...
    pred_path  = 'model/resnet18/result.txt'
